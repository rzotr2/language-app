...
export const QuizExercise = (
    {quizCards, generateMore, difficulty}: QuizExerciseProps
) => {
    ...
    return (
        <>
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 py-2 px-3 sm:py-3 sm:px-6 rounded-t-xl"
                ...
                <p className="text-blue-100 mt-1 text-sm sm:text-lg">Choose the correct answer from suggested options</p>
            </div>
            <div className="bg-gradient-to-r from-blue-800 to-indigo-800 p-2 sm:p-4">
                <div className="space-y-1 text-sm md:text-lg p-2 sm:p-3 font-medium gap-3 bg-gradient-to-r
                    from-blue-500 to-indigo-500 mt-4 rounded-xl">
                    <QuizCardSlider activeSlide={slideNumber}>
                        {quizCards.map((card, index) => {
                            return (
                                <QuizCardSlide card={card} key={index} number={index} />
                            )
                        })}
                    </QuizCardSlider>
                </div>
                <div className="flex justify-between items-center my-4 sm:mx-3 text-white">
                    {slideNumber === 0 ? (
                        <button type="button" onClick={handlePerv} disabled={true}
                        ...
                        <button type="button" onClick={handlePerv}
                    ...
                    {slideNumber === quizCards.length - 1 ? (
                        <button type="button" onClick={handleNext} disabled={true}
                                className="text-gray-900 bg-gray-300 border border-gray-300 focus:outline-none
                                    cursor-not-allowed font-medium rounded-lg text-sm py-1 w-[70px] sm:w-[100px] sm:py-2">
                            Next
                        </button>
