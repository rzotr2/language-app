...
export const BlanksSelectAnswer = ({showExplanation, task}: BlanksSelectAnswerProps) => {
    ...
    return (
        <Select.Root disabled={showExplanation}>
            {showExplanation ? (
                <Select.Trigger
                    className="text-sm sm:text-lg bg-gray-300 px-2 rounded-md flex items-center gap-2 text-red-700">
                    <Select.Value placeholder="_____" className="font-medium " />
                </Select.Trigger>
            ): (
                <Select.Trigger
                    className="text-sm sm:text-lg bg-gray-300 px-2 rounded-md flex items-center gap-1">
                    <Select.Value placeholder="_____" className="font-medium" />
                    ...
                    <Select.Viewport>
                        {task.options.map((option, idx) => (
                            <Select.Item key={idx} value={option}
                                ...
                                {!showExplanation && (
                                    <Select.ItemText
                                        className="font-medium">{option}</Select.ItemText>
                                )}
                            </Select.Item>
                        ))}
                    </Select.Viewport>
