...
import type { BlanksArray } from "../../../types";
...
export const FillTheBlanks = (
    {blanksArray, generateMore, difficulty}: FillTheBlanksProps
) => {
    ...
    return (
        <>
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 py-2 px-3 sm:py-3 sm:px-6 rounded-t-xl"
                ...
                <p className="text-blue-100 mt-1 text-sm sm:text-lg">Complete the sentence by selecting the correct words</p>
            </div>
                        ...
                        <div key={index}>
                            <div className="flex flex-wrap gap-[5px] text-[1rem] items-center">
                                {parts.map((word, index) => {
                                    if (word === "___" || word === "___." || word === "___," || word === "___?" || word === "___!") {
                                        return (
                                            <div key={index}>
                                                <BlanksSelectAnswer task={reset ? null : task} showExplanation={showExplanation} />
                                            </div>
                                        )
                                    } else {
                                ...
                                {showExplanation && (
                                    <CustomTooltip type="hint" text={task.explanation} />
                                )}
                            </div>
